...
// MastershipFilter activates a dispatcher on acquiring mastership
// The MastershipFilter requires a MastershipKeyResolver to extract a device ID from each request. Given a device
...
func (f *MastershipFilter) Accept(id Request) bool {
	key, err := f.Resolver.Resolve(id)
	if err != nil {
		return false
	}
