...
func TestController(t *testing.T) {
	...
	filter.EXPECT().
		Accept(gomock.Any()).
		DoAndReturn(func(request Request) bool {
			i, _ := strconv.Atoi(request.ID)
			return i%2 == 0
		}).
		AnyTimes()
	...
	watcher.EXPECT().
		Start(gomock.Any()).
		DoAndReturn(func(ch chan<- Request) error {
	...
	partitioner.EXPECT().
		...
		DoAndReturn(func(request Request) (PartitionKey, error) {
			i, _ := strconv.Atoi(request.ID)
	...
	wg.Wait()
	watcherCh := watcherValue.Load().(chan<- Request)
	watcherCh <- Request{"1"}
	watcherCh <- Request{"2"}
	watcherCh <- Request{"3"}
	watcherCh <- Request{"4"}
