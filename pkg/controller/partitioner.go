...
type WorkPartitioner interface {
	...
	Partition(request Request) (PartitionKey, error)
}
...
func (p *UnaryPartitioner) Partition(request Request) (PartitionKey, error) {
...
func (p *RegexpPartitioner) Partition(request Request) (PartitionKey, error) {
	return PartitionKey(p.Regexp.FindString(string(request.ID))), nil
}
