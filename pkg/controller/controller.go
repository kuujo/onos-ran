...
func (c *Controller) startWatcher(ch chan Request, wg *sync.WaitGroup, watcher Watcher) error {
...
func (c *Controller) processEvents(ch chan Request) {
	c.mu.RLock()
	filter := c.filter
...
func (c *Controller) partition(id Request, partitioner WorkPartitioner) {
	iteration := 1
	for {
...
func (c *Controller) processRequests(ch chan Request) {
	c.mu.RLock()
...
func (c *Controller) requeueRequest(ch chan Request, id Request) {
	ch <- id
}

// reconcile reconciles the given request ID until complete
func (c *Controller) reconcile(request Request, reconciler Reconciler) Result {
	iteration := 1
	for {
